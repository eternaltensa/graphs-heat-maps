...
def doc():
    ...
    # Определите функцию для добавления изображения с черным фоном
    def add_image_with_black_background(c, image_path, x, y, width, height):
        c.setFillColorRGB(0, 0, 0)  # Устанавливаем черный фон
        c.rect(x, y, width, height, fill=1)  # Рисуем черный прямоугольник
        c.drawImage(image_path, x, y, width, height)

    # Создайте список изображений и их координат
    images = [
    ...
    # Добавьте изображения с черным фоном в документ
